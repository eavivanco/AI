...
class LightPuzzle:
    ...
    def initialize_pdb(id):
        ...
        line = line.rstrip()
        numbers = line.split(' ')
        LightPuzzle.pdb_pattern[id] = [int(x) for x in numbers]  # los valores incluidos en el patron
        while f:
            line = f.readline(100)
            line = line.rstrip()
            numbers = line.split(' ')
...
#            print(tup, value)
    ...
    def light_manhattan(self): ### por implementar
        '''
        ...
        for i in range(0, self.size):
            if self.board[i] == 0:
                ...
                num += abs(i % self.x - self.board[i] % self.x) / (i + 0.1)
                #print(f'este es el num 1 : {num}')
                num += abs(i // self.x - self.board[i] // self.x) / (i + 0.1)
                #print(f'este es el num 2 : {num}')
        return num
    ...
    def manhattan_div_15(self): ### por implementar
        bruto = self.manhattan()
        procesada = bruto / 15
        return procesada
