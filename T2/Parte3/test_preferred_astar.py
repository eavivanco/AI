...
#num_problems = 11            # solo ejecutamos los primeros 10 problemas
for prob in range(0, num_problems):
    init = problems[prob]
    s = PrefAstar(init, heuristic, 50, 100, 2)
    result = s.search()
    if result.g == 0:
        #print('%5d%10d%10d%10d%10.2f%10.2f' % (prob+1, 0, 0, 0, 0, 0)) ## Se implemento el if para hacer evidente que no cumplio el criterio
